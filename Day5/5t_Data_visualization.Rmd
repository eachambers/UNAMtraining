...
library(ggtree)
library(tidyverse)
setwd("~/Box Sync/Rana project/TRAINING_COURSE/Course_materials/Day5") # set this to whichever directory has this markdown file and associated data files are downloaded to
```
...
```{r}
# Import data file
stats <- read_tsv("Day5_datavis_worksheet_files/rana_stats.txt", col_names = TRUE)
...
Let's take a look at amounts of missing data now. The other file (`rana_missing_data.txt`) takes these same samples we've been working with and has the number of sites and percentage of missing data.
...
3. Outline AND fill the bars (two separate functions, see if you can figure this out on your own) according to sample ID and remove the x axis labels, title, and ticks.
```{r}
# your code here
```
...
For `ggtree()`, we're going to use the Nexus file that you created from the Day 4 RAxML worksheet. If you haven't had a chance to do this yet, download the relevant tree files from [here](https://utexas.box.com/s/m7fuv6po4edwqi2k2ak9r7p7i9atwdj8) and go to the part of the 4t_Phylo_tutorial.docx file called "Make a Nexus tree file".

To recap, this Nexus file should contain the Rana ddRAD tree *with support values and branch lengths*. Move (or copy) that file to the current working directory. The file should be called `ranaddrad_tree.nexus`.


As we discussed in lecture, you need to import the Nexus file as a `phylo object`, using the `ape()` package. To do this:
```{r}
# Import the tree
tree <- read.nexus("ranaddrad_tree.nexus")
...
4. Now, reroot the tree at the correct node. We can do this using the `root()` function within `ape` with the syntax `newtree <- root(tree, node=X, edgelabel=TRUE)`. `edgelabel` retains the branch lengths.
```{r}
# your code here
```
...
11. You can export your tree in a few ways, but we'll use the `ggsave` function within `ggplot2`.
```{r}
ggsave("ranaddrad_tree.pdf", plot_tree)
# This will save the PDF to your current working directory.
```
...
1. Using your properly rooted phylo object (`newtree`), build a new tree plot without any information at the nodes (labels or node points). Name this `tree_nonodes`.
```{r}
# your code here
```
